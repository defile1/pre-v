...
let schema =  new Schema(descriptors.ServerSide.Services.Base ,{
...
}, "Only 6 images are allowed to be uploaded per Service.", "images length");
...
schema.virtual("slug").get(function(){
    ...
    return `Services/${YY}/${mm}/${slugify(this.title)}/${this.id}`
})

let Services     = mongoose.model('Services', schema);

var out = {
    Base: Services
}

Enums.Services.categories.forEach((item, index) =>{
    let category = item.key;
    let descriptor = descriptors.ServerSide.Services[category];
