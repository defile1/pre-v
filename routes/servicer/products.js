...
Controller.use(function(req, res, next){
    res.locals.Stores.Descriptors.servicer = Collections.Descriptors.BrowserSide.servicer
...
.get('/search/', function(req, res, next){
    ...
    if(req.query.search){
        options["$text"] = { $search: req.query.search,$caseSensitive: true, $language:"none"};
    }
    ...
    console.log(options);
    Collections.Schemas.servicer.Base.find(options)
    .then((results)=>{
        res.locals.title         = "Manage products";
        res.locals.searchQuery   = req.query.search;
        res.locals.Stores.Data.servicer = {schemaName: "servicer", data: results};
        res.render('./servicer/products')
    })
    .catch(next)
})
.get('/:category/new/', function(req, res, next){
    ...
    let doc = new Collections.Schemas.services[category]({category:human},{strict: false});
    doc.owners.addToSet(req.user.id);
    doc.save({ validateBeforeSave: false })
    .then((results)=>{
        res.redirect(`/servicer/products/${category}/${results._id}/`);
    })
    .catch(next)
})
.get('/:category/', function(req, res, next){
    ...
    Collections.Schemas.Services.Base
    .find(options)
    .then((results)=>{
        res.locals.title         = "Manage products";
        res.locals.Stores.Data.Services = {schemaName: "Services", data: results};
        res.render('./servicer/products')
    })
    .catch(next)
})
.get('/:category/:id/', function(req, res, next){
    ...
    Promise
    .all([
        Collections.Schemas.servicer.Base.findOne(options),
        Collections.Queries.Merchants.Products.getBrands()
    ])
    .then((results)=>{
        let [servicer, brands] = results
        if(servicer===null) return res.redirect(`/servicer/products/${slug}/`);
        if(servicer && servicer.images && servicer.images.length) servicer.images = (servicer.images).filter(function(image){ return image['deleted'] !== void 0 && (image.deleted===false) })
        // Helpers.removeDeleted(product, "images");
        // res.locals.Stores.Data.Products = {schemaName: "Products", data: [product]};
        res.locals.title   = `Manage ${servicer.category} products`;
        res.locals.servicer = servicer;
        res.locals.brands  = brands.map(item => item.toJSON() );

        res.locals.Stores.Data.Brands        = {schemaName: "Brands", data: brands }

        res.render('./servicer/product')
    })
    .catch(next)
})
.get('/', function(req, res, next){
    ...
    Collections.Schemas.Services.Base.find(options)
    .then(results => {
        res.locals.title         = "Manage products";
        res.locals.Stores.Data.Services = {schemaName: "Products", data: results};
        res.render('./servicer/products')
    })
    .catch(next)
})
.get('/:id', function(req,res,next){
    ...
    .all([
        productsID == "new" ?
            new Collections.Schemas.services :
            Collections.Queries.Servicer.Services.getServiceById(serviceID, ownerID),
            Collections.Queries.Servicer.Servicer.getBrands()
        ])
    .then((results) => {
